...
export function initLanguageClient(monaco: any, editorInstance: any, options: any = {}) {
  const {
    socketUrl = `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}/ws/clangd/`,
    //socketUrl = 'wss://olimpiada.ic.unicamp.br/ws/clangd/',
    languages = ['cpp', 'c'],
    debounceDelay = 300,
    maxConcurrentRequests = 2,
    workspaceRoot = 'file:///home/olimpinf/clangd_workspaces/default'
  } = options;

  let messageId = 1;
  let initialized = false;
  const webSocket = new WebSocket(socketUrl);
  const documentUri = `${workspaceRoot}/main.cpp`;
